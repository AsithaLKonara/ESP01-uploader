...
void handleRoot() {
  String page = "<html><head><title>WS2812 LED Strip Uploader</title>"
                ...
                "⚠️  <strong>File Size Limit:</strong> Maximum 700KB for stable operation"
                "</p>"
                "</div>";
...
File uploadFile;
#define UPLOAD_BUFFER_SIZE 2048  // Larger buffer for ESP-01
#define MAX_FILE_SIZE 716800     // 700KB max file size
...
void handleFileUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    if (upload.totalSize > MAX_FILE_SIZE) {
      ...
      server.send(413, "text/plain", "File too large. Maximum size: 700KB");
      return;
    }
    ...
    if (uploadFile) {
      ...
      // Progress indicator every 10KB with memory status
      if (lastUploadedSize % 10240 == 0) {
